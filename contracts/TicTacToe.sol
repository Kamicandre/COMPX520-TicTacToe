...
contract TicTacToe{
    ...
    event GameOverWin(address winner);
    event GameOverDraw();
    event PayoutSuccess(address receiver, uint amountInWei);

    constructor() public payable {
        //Set initial address to player 1
        player1 = msg.sender;
    ...
    function joinGame() public payable{
        //Use to make sure only one person can join the game
        assert(player2 == address(0));
        //Set game to active
        gameActive = true;

        require(msg.value == gameCost);
        //Set address of player 2 to the new address
        player2 = msg.sender;
    ...
    function withdrawWin() public {
      uint balanceTransfer;
        if(msg.sender == player1) {
            require(balanceToWithdrawPlayer1 > 0);
            balanceTransfer = balanceToWithdrawPlayer1;
            balanceToWithdrawPlayer1 = 0;
            player1.transfer(balanceTransfer);
            emit PayoutSuccess(player1, balanceTransfer);
        }
        else{
            require(balanceToWithdrawPlayer2 > 0);
            balanceTransfer = balanceToWithdrawPlayer2;
            balanceToWithdrawPlayer2 = 0;
            player1.transfer(balanceTransfer);
            emit PayoutSuccess(player2, balanceTransfer);
        }
    }
    ...
    function emergencyCastOut() public {
    ...
    function setStone(uint8 x, uint8 y) public {
        ...
        assert(x < boardSize);
        assert(y < boardSize);
        //Check if the active player is correct
